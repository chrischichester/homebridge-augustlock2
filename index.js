      ...
      if (!error && request.statusCode == 200) {
          ...
          self.platformLog("Getting to 410");
             var thisDeviceID = locks.LockID.toString();
              ...
              var thisModel = "August Lock Gen 2/3";
                  // locks.Bridge.deviceModel.toString();
...
console.log(augustState);
  var status = self.lockState[state];
    self.platformLog("543 "+ self.lockids[0]+ " " + augustState);
    request.put({
          url: "https://api-production.august.com/remoteoperate/" + self.lockids[0] + "/" + augustState,
          "headers": {
              "Content-Type": 'application/json',
              'x-kease-api-key': '14445b6a2dba',
              'x-august-access-token': this.securityToken,
